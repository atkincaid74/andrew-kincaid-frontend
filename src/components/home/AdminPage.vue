<template>
  <div>
    <v-card max-width="500" class="mx-auto mt-2">
      ...
      <v-card-actions>
        <v-switch label="Paid" v-model="paid"></v-switch>
        <v-spacer></v-spacer>
        <v-switch label="SC" v-model="superContest"></v-switch>
        <v-spacer></v-spacer>
        <v-switch label="P6" v-model="pickSix"></v-switch>
        <v-spacer></v-spacer>
        <v-btn @click="submitEmail" :loading="loading" :disabled="$v.$invalid">
          Submit
        </v-btn>
      </v-card-actions>
    </v-card>
