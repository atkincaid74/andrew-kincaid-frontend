<template>
  <div>
    <v-card :max-width="width" class="mx-auto my-10">
      ...
      <v-card-actions>
        <v-btn @click="toggleCreateUserDialog">Create Account</v-btn>
        <v-spacer></v-spacer>
        <v-btn @click="submitLoginInfo" :loading="loading">Login</v-btn>
      </v-card-actions>
    </v-card>
    <NewUser
...
export default {
  ...
  computed: {
    ...
    passErrors() {
      ...
      !this.$v.enteredPass.minLength &&
        errors.push("Password must be at least 6 characters");
      !this.$v.enteredPass.maxLength &&
