<template>
  <v-dialog v-model="display" max-width="600" class="mx-auto" persistent>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn @click="closeDialog" color="red">Close</v-btn>
        <v-btn
          color="primary"
          @click="createNewUser"
  ...
  computed: {
    ...
    usernameErrors() {
      ...
      !this.$v.username.maxLength &&
        errors.push("Username must be less than 16 characters");
      return errors;
    },
    passwordErrors() {
      const errors = [];
